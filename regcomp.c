	  ...
	  reparse:
		...
		case '{':
		    ...
		    if (len || (p > RExC_start && isALPHA_A(*(p -1)))) {
                        ...
                        /* This is a total kludge.  GNU Autoconf is depended on
                         * by a lot of code, and can't seem to release a new
                         * version that avoids the deprecation now made fatal.
                         * (A commit to do so has been in its repository since
                         * early 2013; only one pattern is affected.)  As a
                         * work-around, don't fatalize this if the pattern
                         * being compiled is the precise one that trips up
                         * Autoconf.  AndSee [perl #130497]. */
